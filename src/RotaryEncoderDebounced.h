...
class RotaryEncoderDebounced {
private:
    /* data */
    // clk pin to encoder map
    static RotaryEncoderDebounced<clk_pin, dt_pin>* map[11];
...
public:
    ...
    RotaryEncoderDebounced(bool active_low = true);
    ~RotaryEncoderDebounced();
    virtual void begin();
    virtual int read();
};

template<uint8_t clk_pin, uint8_t dt_pin>
RotaryEncoderDebounced<clk_pin, dt_pin>* RotaryEncoderDebounced<clk_pin, dt_pin>::map[11] = {0};

/**
 * @brief Construct a new RotaryEncoderDebounced<clk_pin, dt_pin>::RotaryEncoderDebounced object
 * 
 ...
 */
template<uint8_t clk_pin, uint8_t dt_pin>
RotaryEncoderDebounced<clk_pin, dt_pin>::RotaryEncoderDebounced(bool active_low)
: active_low(active_low), state(NO_ACTION), clk_val(1), dt_val(1) {
    map[clk_pin] = this;
}

template<uint8_t clk_pin, uint8_t dt_pin>
RotaryEncoderDebounced<clk_pin, dt_pin>::~RotaryEncoderDebounced() {
    map[clk_pin] = 0;
 ...
 */
template<uint8_t clk_pin, uint8_t dt_pin>
void RotaryEncoderDebounced<clk_pin, dt_pin>::begin() {
 ...
 */
template<uint8_t clk_pin, uint8_t dt_pin>
int RotaryEncoderDebounced<clk_pin, dt_pin>::read() {
    int oldState = state;
    state = NO_ACTION;
    return oldState;
}
 ...
 */
template<uint8_t clk_pin, uint8_t dt_pin>
void RotaryEncoderDebounced<clk_pin, dt_pin>::clk_trigger_isr() {
    RotaryEncoderDebounced<clk_pin, dt_pin>* encoder = map[clk_pin];
 ...
 */
template<uint8_t clk_pin, uint8_t dt_pin>
void RotaryEncoderDebounced<clk_pin, dt_pin>::dt_trigger_isr() {
    RotaryEncoderDebounced<clk_pin, dt_pin>* encoder = map[clk_pin];
